...
in the row. `sort_order IS NULL` means browseable-only (future 13 (Meal templates)). The partial
...
| `SettingsScreenTest` (new) | "Meal buttons" row is present and navigates to `MealButtonsScreen` — open this test class now per manifesto rule; full Settings coverage to follow in 16 (Past-day view) |
