...
|   | 4  | **Multi-user** — V4/V5 DB migrations add `user_id` to all data tables; all API queries scoped per session; Android SyncWorker + login sync; Ansible provisions users from vault | Multiple users; data isolated at DB layer; history restores on login | [multi-user-sync-design](multi-user-sync-design.md) |
