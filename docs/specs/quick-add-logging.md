...
- **`DashboardLogicTest`** (extend): verify that the caloriesIn reactive update correctly
  excludes `PENDING_DELETE` entries and entries from other days.

### Tier 2a — server integration

- **`LogEntryIntegrationTest`** (new, UUID slot #4 — `...000004`,
  `logentry-test@test.local`):
  - `POST /in/log/quick-add` → 201, body matches request fields.
  - `GET /in/log` → returns the created entry.
  - `DELETE /in/log/{id}` → 204; subsequent GET excludes it.
  - `DELETE /in/log/{id}` for another user's entry → 404.
  - Duplicate `POST` with same UUID → 409 (or idempotent 201 if server upserts).
  - `GET /summary/today` `caloriesIn` reflects the quick-add entry.

### Tier 2b — app component

- **`LogEntryDaoTest`** (extend): `deleteById` removes the row; `sumQuickAddKcalForDate`
  only counts today's date, excludes entries with null `quickAddKcal`.
- **`LogScreenTest`** (new): Add button disabled when kcal field empty; entry appears
  in list after valid add; delete sheet shown on entry tap; undo after add removes entry.
- **`SyncWorkerTest`** (new, Robolectric + in-memory Room + `MockEngine`):
  - `PENDING_CREATE` entry: worker POSTs to `/in/log/quick-add`, marks `SYNCED` on 201.
  - `PENDING_CREATE` entry: worker leaves status unchanged on network error.
  - `PENDING_DELETE` entry (was `SYNCED`): worker DELETEs from server, hard-deletes
    from Room on 204.
  - `PENDING_DELETE` entry: worker hard-deletes from Room on 404 (already gone).

### Tier 3 — API test

- **`LogEntryApiTest`** (new): POST quick-add, GET /in/log confirms entry, DELETE,
  GET /in/log confirms removal. Runs against real production server.

### Tier 4 — E2E smoke (first suite)

First E2E smoke test, now that both Dashboard and Log screens exist:
- Login → Dashboard shows budget → navigate to Log tab → add quick entry → navigate
  back to Dashboard → verify `caloriesIn` updated → sign out.
- Lives in `app/src/androidTest/`. Uses `test+e2e@bran.name` account.
- Any entry written during the test is deleted in `@After`.

---

## Docs updated alongside this story

- `docs/specs/api-design.md` — add `POST /in/log/quick-add` and `DELETE /in/log/{id}`
  to the endpoint table; correct the `GET /summary/today` DTO shape (spec currently
  diverges from implementation).
- `docs/specs/testing-manifesto.md` — add `LogEntryIntegrationTest` to UUID registry
  (slot #4); update "what currently meets the bar" for Tier 2a, 2b, 3 once implemented.
- `docs/specs/feature-backlog.md` — mark story 7 done once implemented.
