...
|   | 12 | **Budget model swap** — replace time-decay + eating-fraction with simple running balance (`calories_out − D − calories_in`); remove per-minute tick; simplify DTO + Room entity  | Budget correctly shows remaining headroom at any time of day                 | [budget-model-swap](specs/budget-model-swap.md) |
...
|   | 17 | **Weekly verdict** — on-track / behind / dropping-fast, driven by smoothed weight trend                                                                                         | The honest weekly signal                                             |                                           |
|   | 18 | **Weight trend chart** — 7-day smoothed line on dashboard                                                                                                                       | Visual progress                                                      |                                           |
|   | 19 | **Calorie balance bars** — weekly in/out bars on dashboard                                                                                                                      | Over/under pattern visible at a glance                               |                                           |
|   | 20 | **Widget (small)** — verdict dot + budget remaining on homescreen                                                                                                               | One-glance status without opening the app                            |                                           |
|   | 21 | **Widget (medium)** — adds weight sparkline + drink shortcuts row to widget                                                                                                     | One-tap drinks from homescreen                                       |                                           |
|   | 22 | **Auto update** — server serves version manifest + APK; app checks on launch, downloads via DownloadManager, installs via system sheet; deploy script                           | Ship a new build and the app updates itself next launch              | [auto-update](auto-update.md)             |
|   | 23 | **End-of-day notification** — gentle local reminder when dinner unlogged (~21:00)                                                                                               | Forgetting-to-log caught without guilt                               |                                           |
|   | 24 | **Insights** — late-snack frequency, drink clusters, missing-log coverage, calorie-vs-weight drift note                                                                         | Behavioural patterns visible without moralising                      |                                           |
|   | 25 | **Vacation mode** — pause tracking, exclude period from verdicts and pattern calculations                                                                                       | Holidays don't skew the trend                                        |                                           |
|   | 26 | **Maintenance mode** — stability verdict after goal reached, budget switches to balance                                                                                         | Success state has a proper mode                                      |                                           |
