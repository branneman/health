...
|   | 17 | **Quick single-item log** — two new log-sheet entries: "Single item" (search food → grams → log) and "Scan & log" (barcode scan → grams → log); no template, no builder, ideal for snacks | One-tap snack logging without knowing the kcal number upfront        |                                           |
|   | 18 | **Past-day view** — browse history, swipe between days, delete entries                                                                                                          | Corrections and history visible                                      |                                           |
|   | 19 | **Weekly verdict** — on-track / behind / dropping-fast, driven by smoothed weight trend                                                                                         | The honest weekly signal                                             |                                           |
|   | 20 | **Weight trend chart** — 7-day smoothed line on dashboard                                                                                                                       | Visual progress                                                      |                                           |
|   | 21 | **Calorie balance bars** — weekly in/out bars on dashboard                                                                                                                      | Over/under pattern visible at a glance                               |                                           |
|   | 22 | **Widget (small)** — verdict dot + budget remaining on homescreen                                                                                                               | One-glance status without opening the app                            |                                           |
|   | 23 | **Widget (medium)** — adds weight sparkline + drink shortcuts row to widget                                                                                                     | One-tap drinks from homescreen                                       |                                           |
|   | 24 | **Auto update** — server serves version manifest + APK; app checks on launch, downloads via DownloadManager, installs via system sheet; deploy script                           | Ship a new build and the app updates itself next launch              | [auto-update](auto-update.md)             |
|   | 25 | **End-of-day notification** — gentle local reminder when dinner unlogged (~21:00)                                                                                               | Forgetting-to-log caught without guilt                               |                                           |
